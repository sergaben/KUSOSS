...
class KingstonStudentRepositoryImpl @Inject()(protected val dbConfigProvider:DatabaseConfigProvider,kingstonStudentSchema: KingstonStudentSchema)
  ...
  override def updateOrInsertToken(id:Option[Int],nickname: String,email:String,password:String,subject:String,typeOfStudy:String,tokenToBeInserted:Option[String]): Future[Int] = {
    ...
    val getResult = for{
      ...
      checkIfStudentHaveToken:Option[String] = row.loginToken.getOrElse(tokenLoginAsOption) // this gets the token from the previous row if there is no token we create a new one
      finalRow = row.copy(loginToken = checkIfStudentHaveToken) // this creates a new kingston student with the token saved in the previous variable
